...
if [[ "$1" == --parallel=* ]]; then
	...
	PARALLEL=$(nproc 2> /dev/null || echo 10)
fi
...
TOTAL_SIZE=$(awk '{ts+=$1}END{printf "%f.0f", ts}' < "${TMPDIR}/files.all")
...
for ((I = 1 ; I < PARALLEL ; I+=2 )); do
	tac "${TMPDIR}/chunk.${MIN_I}" > "${TMPDIR}/chunk.${MIN_I}.r" && mv "${TMPDIR}/chunk.${MIN_I}.r" "${TMPDIR}/chunk.${MIN_I}"
done
